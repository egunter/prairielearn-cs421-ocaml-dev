...
%{
    ...
    let leqsugar l r = (BinOpAppExp(GreaterEqOp, r, l))
    let geqsugar l r = (BinOpAppExp(GreaterEqOp,l,r))
    let neqsugar l r = (BinOpAppExp (NeqOp,l,r))
(*
    let and_ext_val_sugar l r mem = IfVal(l, r, ConstExp FalseConst, mem)
    let or_ext_val_sugar l r mem = IfVal(l,ConstExp TrueConst, r, mem)

    let lt_ext_val_sugar l r = BinOpAppVal(GreaterOp,r,l)
    let leq_ext_val_sugar l r = or_ext_val_sugar (lt_ext_val_sugar l r) (BinOpAppVal(EqOp, l, r))
    let geq_ext_val_sugar l r = (BinOpAppVal(GreaterOp,l,r))
    let                           (BinOpAppExp(EqOp, l, r))
    let neqsugar l r = (BinOpAppExp (NeqOp,l,r), ConstExp FalseConst,
    		       			 ConstExp TrueConst)
      *)
...
%token TRUE FALSE NEG PLUS MINUS TIMES DIV DPLUS DMINUS DTIMES DDIV MOD EXP CARAT
       ...
       UNDERSCORE UNIT HEAD TAIL PRINT FST SND EOF EVAL VAL BY

/* Define the "goal" nonterminal of the grammar: */
%start environment interactive expression
/*
%type <Picoml_exp_and_val.dec> dec
%type <(string * Picoml_exp_and_val.value) list> environment
%type <(string * Picoml_exp_and_val.value) list list> environment_list
%type <(Picoml_exp_and_val.exp * string) option> interactive
  */
    
%type <Picoml_exp_and_val.exp> expression
%type <(string * Picoml_exp_and_val.value) list> environment
%type <Picoml_exp_and_val.exp option> interactive
%%

interactive:
DARROW expression /* BY RULE */                              { Some ($2) }
...
/*
environment_list:
    environment environment_list       { $1::$2 }
  | environment                        { [$1] }
*/
...
rel_exp:
  ...
  | pure_rel_exp LT cons_exp		{ BinOpAppExp (GreaterOp,$3,$1) }
  | pure_rel_exp GEQ cons_exp		{ BinOpAppExp (GreaterEqOp,$1,$3) }
  | pure_rel_exp LEQ cons_exp		{ BinOpAppExp (GreaterEqOp,$3,$1) }
  | pure_rel_exp EQUALS cons_exp	{ BinOpAppExp (EqOp,$1,$3) }
  | pure_rel_exp NEQ cons_exp		{ BinOpAppExp (NeqOp,$1,$3) }
  | cons_exp	     			{ $1 }
...
no_try_rel_exp:
  ...
  | pure_rel_exp LT no_try_cons_exp	{ BinOpAppExp (GreaterOp,$3,$1) }
  | pure_rel_exp GEQ no_try_cons_exp	{ BinOpAppExp (GreaterEqOp,$1,$3) }
  | pure_rel_exp LEQ no_try_cons_exp	{ BinOpAppExp (GreaterEqOp,$3,$1) }
  | pure_rel_exp NEQ no_try_cons_exp	{ BinOpAppExp (NeqOp,$1,$3) }
  | no_try_cons_exp    			{ $1 }
...
pure_rel_exp:
  ...
  | pure_rel_exp GEQ pure_cons_exp	{ BinOpAppExp (GreaterEqOp,$1,$3) }
  | pure_rel_exp LEQ pure_cons_exp	{ BinOpAppExp (GreaterEqOp,$3,$1) }
  | pure_rel_exp NEQ pure_cons_exp	{ BinOpAppExp (NeqOp,$1,$3) }
  | pure_cons_exp	     		{ $1 }
...
atomic_expression:
  ...
  | EVAL LPAREN expression COMMA environment RPAREN  { Eval ($3, $5) }
  | VAL value       { Val $2 }
