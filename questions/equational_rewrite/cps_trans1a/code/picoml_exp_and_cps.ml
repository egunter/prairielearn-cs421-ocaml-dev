...
and exp_cps =
 ...
 | CPS_Trans of exp * cps_cont                            (* [[exp]]_k *)
...
let rec freeVarsInExpCPS cont =
    ...
    | CPS_Trans (e, k) -> (freeVarsInExp e) @ (freeVarsInContCPS k)
and
   freeVarsInContCPS k =
   match k with External -> [] 
   | ContMetaVar -> []
   | ContVarCPS _ -> []
   | FnContCPS (k, e) -> (List.filter (fun y -> not(y = k)) (freeVarsInExpCPS e))
...
let rec exp_cps_norm_aux var_subst free_vars exp_cps =
 ...
 | CPS_Trans (exp, kappa) -> CPS_Trans(exp, cps_cont_norm_aux var_subst free_vars kappa)

and cps_cont_norm_aux var_subst free_vars kappa =
 match kappa with
   External -> External
 | ContMetaVar -> ContMetaVar
...
let exp_cps_equiv ec1 ec2 =
  ((exp_cps_normalize ec1 (freeVarsInExpCPS ec1)) =
      (exp_cps_normalize ec2 (freeVarsInExpCPS ec2)))
