...
type ('judgment, 'obj, 'source, 'sc_error) problem_def =
  ...
  *)
	...
	(* raw node id, label, judgment *)
type 'judgment raw_tree = (string * (string * 'judgment)) list

type 'judgment proof_tree = ProofTree of 'judgment proof_tree list * string * 'judgment
...
let rec string_tree_to_proof_tree (plist: 'judgment raw_tree) (StringTree(s, sl)): 'judgment proof_tree = match lookup_env plist s with
	...
	| Some (lbl, judgment) ->
		...
		in ProofTree(child_list, lbl, judgment)
  ...
  TermElem("judgment", (TermElem(x, _, _)) :: _, _) ->
    let rec td_find_rule rule_list = match rule_list with
      ...
      | (pts, SynRule(l_g, n, p, pl, sc)) :: rule_t ->
        if x = n then (l_g, SynRule(l_g, n, p, pl, sc)) :: td_find_rule rule_t else td_find_rule rule_t
...
let rec check_rule pdef (lbl, j_term, sub_term_list) (rule_name, c_rule) =
  let c_err = match_rule c_rule [] pdef.eq_poly_fun (lbl, j_term, sub_term_list)
  in (rule_name, c_err)
...
let check_rule_list pdef (lbl, j_term, sub_term_list) rule_list =
  get_shortest_list (List.map (check_rule pdef (lbl, j_term, sub_term_list)) rule_list)

  (* collects errors from a proof tree *)
let rec check_tree (pdef: ('judgment, 'obj, 'source, 'sc_error) problem_def)
	(ProofTree(pl, lbl, judgment): 'judgment proof_tree): ('judgment, 'obj, 'source, 'sc_error) node_error list =
  let sub_plist = List.map (fun (ProofTree(_, _, j)) -> j) pl
  in let sub_jlist = List.map pdef.judgment_to_term sub_plist
  in let (jt, sub_tlist) = (pdef.judgment_to_term judgment, term_list_collect sub_jlist)
  in match (jt, sub_tlist) with
    | (JudgeParse j_term, JParseList sub_term_list) ->
      let rule_list = select_rule_list pdef j_term
      in let (rule_name, c_err) = check_rule_list pdef (lbl, j_term, sub_term_list) rule_list
      ...
      in [NodeErr(c_err, Some rule_name, (sub_plist, judgment))] @ (check_tree_list pdef pl)*)
    | (JudgeErr j_err, _) -> [NodeErr([TParseError j_err], None, (sub_plist, judgment))]
    | (JudgeParse j_term, JErrList) ->
      let j_err = [TSubParseError (string_of_tree_term pdef.string_of_source j_term)]
...
let test_result (pdef: ('judgment, 'obj, 'source, 'sc_error) problem_def)
    ...
    let proc_tree = List.map (fun (key, node) -> (key, (node.str_label, pdef.process node))) tree
