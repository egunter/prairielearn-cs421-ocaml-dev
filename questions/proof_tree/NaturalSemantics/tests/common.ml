...
type eval_result =
    IntResult of int
  | BoolResult of bool
  | MemoryResult of memory
let string_of_eval_result r =
  match r with
    | IntResult int -> string_of_int int
    | BoolResult true -> "true" | BoolResult false -> "false"
    | MemoryResult memory -> string_of_memory memory

type node =
  { label  : label;
    left   : thing_to_eval parsed;
    middle : memory parsed;
    right  : eval_result parsed;
    sideCondition : bool_exp parsed }


let string_of_sideCondition sc = string_of_bool_exp sc

let string_of_node node =
  "{str_label = \""        ^ string_of_label node.label ^ "\"; " ^
  "str_left = \""          ^ string_of_parsed string_of_thing_to_eval node.left ^ "\"; " ^
  "str_middle = \""        ^ string_of_parsed string_of_memory node.middle ^ "\"; " ^
  "str_right = \""         ^ string_of_parsed string_of_eval_result node.right ^ "\"; " ^
  "str_sideCondition = \"" ^ string_of_parsed string_of_bool_exp node.sideCondition ^ "\"}"
