...
type judgment =
  Judgment of (thing_to_eval * memory) parsed * eval_result parsed
...
let string_of_judgment (Judgment(jm, r)) =
    (string_of_parsed string_of_term_mem_pair jm) ^ " evals to "^(string_of_parsed string_of_eval_result r)
  (*
...
let process_judgment (node: unprocessed_node): judgment =
  ...
  in let r = process "eval_result"  eval_result node.str_middle
  in Judgment(jm, r)