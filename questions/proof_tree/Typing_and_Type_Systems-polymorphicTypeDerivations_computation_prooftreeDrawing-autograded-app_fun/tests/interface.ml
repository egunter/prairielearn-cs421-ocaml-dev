...
let rec term_list_collect (sl: ('obj, 'source) term_parse_error list): ('obj, 'source) term_parse_elist = match sl with
  ...
  (*
    ...
    guess_index: the index of the term to use in the initial judgment if the label is left unknown
...
type ('judgment, 'obj, 'source, 'sc_error) problem_def =
    ...
    string_of_failed_sc: string -> 'sc_error -> string
	}
...
let select_index full_list i =
  let rec si_rec l i = match l with
    [] -> List.hd full_list
    | x :: t -> if i = 0 then x else si_rec t (i - 1)
  in si_rec full_list i

let term_guess t = match t with
  TermElem(x, _, _) -> x
  | _ -> "none"

  (* selects all relevant rules from the rule list *)
let select_rule_list pdef t = match t with
  TermElem("judgment", term_list, _) ->
    let s_term = select_index term_list pdef.guess_index
    in let x = term_guess s_term
    in let rec td_find_rule rule_list = match rule_list with
      | [] -> []
  ...
  | TermElem(x, _, _) -> raise (Failure ("BUG: interface - type derivation check being performed on non-judgment \"" ^ x ^ "\""))
  | _ -> raise (Failure "BUG: interface - type derivation check being performed on term object (instead of term judgment).")
...
type possible_points =
  ...
  (*
    takes error information for one node + the set of unused labels,
    returns the max possible points (reason + value), and an updated set of unused labels
  *)
let points_of_node_error pdef (NodeErr(c_err, rule_sel, _)) rem_lbl_list =
  ...
  in match rule_sel with
    ...
    | Some rule_name -> (match take_from_label_list rem_lbl_list rule_name with
      | None -> (PRuleOverused rule_name, err_count, rem_lbl_list)
...
let string_of_error pdef e =
  ...
  (*
    takes the list of errors for one node, max points possible (value), and "effective" points lost
    returns a string containing summary of errors + true points lost
  *)
let string_of_error_list pdef c_err pts_possible pts_lost =
  let (status_start, capped_pts_lost) = match pts_possible with
    | PNoParse -> ("  No points given because rule could not be determined.", pdef.gen_penalty)
    | PRuleOverused x ->
      ("  No points given because max credit for use of rule `" ^ x ^ "` has already been given.",
        min pdef.gen_penalty pts_lost)
    | PRuleValue(x, pts_worth) ->
      let capped_pts_lost = pts_worth - (max 0 (pts_worth - pts_lost))
      in ("  Use of rule `" ^ x ^ "` is worth " ^ (string_of_int pts_worth) ^ " points.", capped_pts_lost)
  in if capped_pts_lost = 0 then ("  No errors!\n" ^ status_start, 0)
...
*)
  ...
  (*
    takes max points possible (value), "effective" points lost, and error information for one node
    returns a string containing all node information (including errors) + true points lost
  *)
...
let string_of_unused_rules pdef rem_lbl_list =
  ...
  (*
    takes the problem's full list of labels + error information for all nodes
    returns a completed error report. notable variables here
      err_report --> complete error report for all nodes
      pts_lost --> sum of all "true" points lost
      pts_lost_rem --> points lost purely from unused labels
      err_end --> error report for unused labels
      score --> final score
  *)
...
in let score = max (max_points - pts_lost - pts_lost_rem) 0
