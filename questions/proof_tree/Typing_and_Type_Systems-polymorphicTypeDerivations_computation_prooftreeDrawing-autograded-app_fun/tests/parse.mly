...
%token TRUE FALSE NEG PLUS MINUS TIMES DIV DPLUS DMINUS DTIMES DDIV EXP CARAT
       ...
       HEAD TAIL PRINT FST SND EOF ALL DOT COLON SATISFIES LBRACE RBRACE
       INSTANCE GEN

/* Define the "goal" nonterminal of the grammar: */
%start expression mono_ty type_env side_condition
%type <Common.exp> expression
%type <Common.monoTy> mono_ty
%type <Common.type_env> type_env
%type <Common.monoTy Genutils.env> side_condition
...
side_condition:
    INSTANCE COLON LBRACE sc_item_list RBRACE         { $4 }
  | INSTANCE COLON LBRACE RBRACE                      { [] }
  | LBRACE sc_item_list RBRACE         { $2 }
  | LBRACE RBRACE                      { [] }
  | sc_item_list                       { $1 }


sc_item_list:
    sc_item COMMA sc_item_list        { $1 :: $3 }
  | sc_item                            { [$1] }

sc_item:
  | TYVAR ARROW mono_ty                { ($1, $3) }
...
env_item:
    IDENT COLON poly_ty                { ($1, $3) }

poly_ty:
   | ALL type_var_list DOT mono_ty { ($2, $4)}
   | mono_ty { ([], $1) }

type_var_list:
   | TYVAR type_var_list { $1 :: $2 }
   | TYVAR { [$1] }

mono_ty:
   | tuple_type ARROW mono_ty { TyConst ("->", [$1; $3]) }
   | tuple_type { $1 }

tuple_type:
   | tuple_list { $1 }
   | instance_or_atomic { $1 }

tuple_list:
   | instance_or_atomic TIMES tuple_list { match $3 with TyConst ("*", v) -> TyConst ("*", $1 :: v) | _ -> raise (Failure "impossible") }
   | instance_or_atomic TIMES instance_or_atomic { TyConst ("*", [$1; $3]) }
...
instance_type:
   | instance_type IDENT { TyConst ($2, [$1]) }
   | instance_args IDENT { TyConst ($2, $1) }

instance_args:
   | atomic_type { [$1] }
   | LPAREN instance_list RPAREN { $2 }

instance_list:
   | mono_ty COMMA mono_ty { [$1; $3] }
   | mono_ty COMMA instance_list { $1 :: $3 }

atomic_type:
   | IDENT { TyConst ($1, []) }
   | LPAREN mono_ty RPAREN { $2 }
   | TYVAR { TyVar $1 }
