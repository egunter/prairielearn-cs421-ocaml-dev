...
# A specialized version of the entrypoint script for the Scheme MP.
...
cp -rv $STUDENT_DIR. $AG_DIR'app/Scheme/'

# Copy the grader script and catch header into the run directory
cp -v $SHARED_DIR'haskell-grader/grader-qc.py' $AG_DIR
...
echo "Starting grading..." > $RESULTS_DIR'log.txt'
...
/usr/bin/sudo -H -u ag bash -c 'python3.6 grader-qc.py' > results.json
